...
citdp.completion_criteria.activation = {
  run_id: "slice2a-verification-2026-09-23",
  phase: "verification",
};
...
const gate = tiedCliJson("tied_checklist_gate_validate", {
  ...
  receipt_persistence: {
    ...
    ledger_path: "working/REQ-TIED_UNIFIED_TOOLCHAIN/gates/gate-ledger.jsonl",
    run_id: "slice2a-verification-2026-09-23",
  },
});
...
const outPath = path.join(
  repoRoot,
  "working/REQ-TIED_UNIFIED_TOOLCHAIN/gates/phase3b-slice2a-verification-gate.json",
);
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, `${JSON.stringify(gate, null, 2)}\n`, "utf8");
console.log(JSON.stringify({ allowed: gate.allowed, diagnostics: gate.diagnostics, path: outPath }, null, 2));
