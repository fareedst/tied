...
| Field | Value |
| --- | --- |
| **REQ** | [REQ-TIED_UNIFIED_TOOLCHAIN](../../tied/requirements/REQ-TIED_UNIFIED_TOOLCHAIN.yaml) (**Implemented** via `tied_verify` 2026-09-23; arc **`close_out`** pending sponsor **`traceable-commit`**) |
...
| **Last delivery** | **2026-09-23 (local)** — Phase **4a–4d** TS-only agentstream; **`tied_verify`** → REQ **Implemented** (pending sponsor commit for arc **`close_out`**) |
...
| **4d** | **Complete (local)** | Go tree removed; oracle fixtures under `mcp-server/packages/agentstream/testdata/`; TS-only `tied agentstream` |

**REQ satisfaction:** **Implemented** (verification-gated 2026-09-23) for Tier 1 suite + Phase **3b** qualified argv + Phase **4** legacy removal (**4a–4d** local). Machine arc **`close_out`** remains pending **`traceable-commit`** and sponsor push policy.
...
| **4** | Legacy removal + optional embed | Slices **4a–4d** (+ optional **4embed**); see Phase 4 section | Full parity; TS default; deprecation window; Go/Ruby removed | **4d complete (local); arc close_out pending traceable-commit** |
...
**Recommended:** Sponsor **`traceable-commit`** (git commit + CHANGELOG), then **`plan-close-out`** or re-run [run-phase4-arc-close-out-gate.mjs](./run-phase4-arc-close-out-gate.mjs) for Phase **4** arc **`close_out`** (`allowed: true` when tree clean). Optional: **`build-plan`** for **4embed** spike only.

**Post–4d process (2026-09-23):** `tied_verify` unblocked via authoritative `tracker_path` + PSA hydration on **`tied_verify`** MCP; **`sub-close-out-evidence-sync`** completed on Tracker; **`traceable-commit`** remains **pending** until sponsor commit (pre-commit evidence in [evidence/traceable-commit-evidence.md](./evidence/traceable-commit-evidence.md)).

**Not recommended:** Claiming machine **`close_out`** before **`traceable-commit`** or push without sponsor approval.
...
| Phase **4** (arc) | **`close_out`** | **`allowed: false`** (pending **`traceable-commit`**) | [phase4-full-close-out-gate.json](./gates/phase4-full-close-out-gate.json); envelope blocking gaps **0**; re-run after sponsor commit |
