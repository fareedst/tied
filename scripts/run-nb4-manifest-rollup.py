#!/usr/bin/env python3
"""NB-4-G partial rollup: refresh wave-4 rows from scan diffs + receipt paths (honest aggregate)."""
...
def main() -> None:
    ...
    for row in doc["clients"]:
        ...
        if row["aggregate_migration_state"] == "fleet-migrated-client":
            row["notes"] = (
                "NB-4 tranche final complete; all active sidecars constraint-enforced-v2; "
                "G3 receipts on file."
            )
        else:
            row["notes"] = (
                "NB-4 tranche final wave complete; G3 receipts on file. "
