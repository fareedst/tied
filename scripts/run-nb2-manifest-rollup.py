#!/usr/bin/env python3
"""NB-2-G partial rollup: refresh wave-2 rows from scan diffs + receipt paths (honest aggregate)."""
...
def main() -> None:
    ...
    for row in doc["clients"]:
        ...
        if row["aggregate_migration_state"] == "fleet-migrated-client":
            row["notes"] = (
                "NB-2 tranche one complete; all active sidecars constraint-enforced-v2; "
                "G3 receipts on file."
            )
        else:
            row["notes"] = (
                "NB-2 tranche one wave complete; G3 receipts on file. "
