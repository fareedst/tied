...
`tools/bundled-prompt-type-skills/ammend-commit/SKILL.md`, including its
...
validation, and parent-handoff contract; do not rely on `@ammend-commit` as
an include. Preserve the spelling `ammend-commit`.

<!-- [IMPL-PROMPT_TYPE_SUBAGENT] [ARCH-PROMPT_TYPE_SUBAGENT] [REQ-PROMPT_TYPE_SUBAGENT] — How: verify the agent file is an explicit-only writable foreground wrapper around the canonical ammend-commit skill. -->
...
4. TIED applicability is explicit because this is `ammend-commit`; do not
...
5. Treat caller-provided Git context as informational only. Do not inspect the
   ...
   caller-supplied ammend git preamble.

## Procedure and gates

Process the request in this order:

1. Apply the caller-supplied git preamble (ammend variant from
   `git-context-templates.md`).
2. **Process** — follow canonical `tied-close-out-process.md`, including the
   ammend-commit prologue: sync IMPL, ARCH, and REQ as needed; update
   CHANGELOG; draft a proposed amend message.
...
- files changed, CHANGELOG entry, and the proposed amend message;
...
failure evidence instead of claiming success. Do not claim that a commit or
amend was created. Gate `allowed: true` alone is not machine close-out.
