...
`tools/bundled-prompt-type-skills/non-tied-plan/SKILL.md`, including its
...
validation, and parent-handoff contract; do not rely on `@non-tied-plan` as
an include.

<!-- [IMPL-PROMPT_TYPE_SUBAGENT] [ARCH-PROMPT_TYPE_SUBAGENT] [REQ-PROMPT_TYPE_SUBAGENT] — How: verify the agent file is an explicit-only writable foreground wrapper around the canonical non-tied-plan skill. -->
...
3. Do not write TIED YAML, CITDP, Tracker, semantic-token, IMPL sidecar, or
   ...
   write tools.
4. TIED applicability is explicit because this is `non-tied-plan`; do not
...
2. **Plan** — follow the plan section of canonical `non-tied-plan.md`.
3. **Implement** — follow the implement section of canonical `non-tied-plan.md`.
   Do not start code until a test strategy is in place. This prompt type does
   not enforce TIED pseudo-code gates through TIED tools.
