...
`tools/bundled-prompt-type-skills/refine-plan/SKILL.md`, including its
...
validation, and parent-handoff contract; do not rely on `@refine-plan` as
an include.

<!-- [IMPL-PROMPT_TYPE_SUBAGENT] [ARCH-PROMPT_TYPE_SUBAGENT] [REQ-PROMPT_TYPE_SUBAGENT] — How: verify the agent file is an explicit-only writable foreground wrapper around the canonical refine-plan skill. -->
...
4. TIED applicability is explicit because this is `refine-plan`; do not
...
Improve the linked plan in this order. If neither a linked plan nor an
in-message plan is present, stop.
