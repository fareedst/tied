...
| `pseudocode_validate` | Layer B structural pseudo-code validation with source-located diagnostics, block discovery, SHAPE-003..006 contract fields, symbol closure, dependencies, token linkage, and optional branch/failure coverage references. It explicitly does not prove behavioral test coverage or runtime correctness. |
| `binding_inventory_validate` | Validate structured binding inventory rows for trigger, callee, arguments, effect, ordering, failure behavior, UI-free composition evidence, and named E2E platform constraints. It does not prove the bound runtime behavior or passing composition tests. |
| `quality_evidence_manifest_build` | Build deterministic `verification-evidence-manifest.v1` output from command results, quality evidence rows, covered tokens, and proof boundaries. Human waiver, residual-risk, owner, and expiry decisions are kept separate. |
| `quality_evidence_collect` | Execute declared argv-only quality commands with timeout/output limits and write bounded stdout/stderr artifacts. |
| `quality_evidence_collect_manifest` | Execute declared commands and build `verification-evidence-manifest.v1` from observed results; it does not treat planned commands as executed evidence. |
| `quality_security_profile_validate` | Validate the external-input security abuse-case set, requiring executable evidence or an owned, expiring waiver. |
| `test_adequacy_validate` | Validate profile-triggered mutation/property/fuzz/flaky/harness/maintainability/dependency and external-call cost plans. It checks control metadata only; it does not run the specialized tests. |
