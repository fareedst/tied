...
| `tdd_state_guide` | TDD progression guide. Params: optional `current_state` as `arguments.current_state`. Terminal state **`end_tdd`** (`is_end`); legacy `end` accepted as alias. States: `S09.RED` … `S11`. |
| `requirement_list_state_guide` | Client-supplied requirement list walk. First call: non-empty `requirements`; later: `current_state` = `continuation_state`. Run until **`id: end_requirement_list`** or error. Per item: full `agent_req_state_guide` through **`end_agent_req`**, strict TDD. See [docs/requirement-list-state-guide-agent-workflow.md](../docs/requirement-list-state-guide-agent-workflow.md). |
| `req_impl_state_guide` | Nested walk: same `requirements` + `continuation_state` contract, but each response is one **spec × one checklist step** (S01…`end_agent_req` per spec). Terminal **`end_req_impl`**. Composite `state` = `specId__stepId`. See [docs/req-impl-state-guide-agent-workflow.md](../docs/req-impl-state-guide-agent-workflow.md). |
| `agent_req_state_guide` | Full agent REQ checklist S01–S16; terminal **`end_agent_req`**. Params: optional `current_state`. |
| `documentation_first_state_guide` | Documentation-first checklist S01–S06.6; terminal **`end_documentation_first`**. |
