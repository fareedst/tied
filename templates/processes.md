...
**`lint_yaml`** / **`yaml_tool`** — Global shell function (or project-provided equivalent) that agents **must** use to validate YAML syntax and canonicalize formatting **in place**. Default canonicalization uses the typed `tied-yaml-canonical-v1` profile: recursive lexical map-key ordering, eligible all-string-list ordering, ordered-list exceptions, scalar-type preservation, atomic writes, and opaque block-scalar/IMPL-sidecar text. Agents use `lint_yaml` / `yaml_tool`, not ad-hoc serializers.
