...
- `scripts/yaml_tool.sh` — Validate and pretty-print TIED YAML (`yq -i -P`, one file per invocation) or sort qualifying list groups (`--sort-lists`, Ruby); supports `-F/--find`, stdin, and NUL-separated paths (see **[PROC-YAML_EDIT_LOOP]** for lint).
