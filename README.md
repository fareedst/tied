...
- `scripts/yaml_tool.sh` — Validate and pretty-print TIED YAML (`yq -i -P`, one file per invocation), sort qualifying list groups (`--sort-lists`, Ruby; 2+ items), or optionally sort map keys (`--sort-keys` with `--sort-lists`); supports `-F/--find`, stdin, and NUL-separated paths (see **[PROC-YAML_EDIT_LOOP]** for lint).
