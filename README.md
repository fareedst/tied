...
**Deeper visuals:** [tied/docs/methodology-diagrams.md](tied/docs/methodology-diagrams.md) — traceability stack, dev cycle, TDD inner loop, CITDP, YAML edit loop.
...
For a full example process of adding the TIED MCP to a project and invoking it in several passes (bootstrap, establish REQ/ARCH/IMPL, maintain), see [tied/docs/adding-tied-mcp-and-invoking-passes.md](tied/docs/adding-tied-mcp-and-invoking-passes.md).

### Using TIED without MCP

If you do not use MCP, run `./bootstrap_without_mcp.sh /path/to/project` to get the same `tied/` layout; then manage YAML by hand or with tools (e.g. `yq`). See [tied/docs/using-tied-without-mcp.md](tied/docs/using-tied-without-mcp.md) for the workflow.
...
2. **Design**: Document architecture in `architecture-decisions.yaml` with `[ARCH-*]` tokens and implementation in `implementation-decisions.yaml` with `[IMPL-*]` tokens; update `semantic-tokens.yaml`. Plan implementation steps in-session or in `implementation-decisions` (checklist-driven work uses `tied/docs/agent-req-implementation-checklist.yaml` via agentstream).
...
For the full agent-executable procedure from sponsor intent through commit, see **[tied/docs/agent-req-implementation-checklist.md](tied/docs/agent-req-implementation-checklist.md)** (executable YAML: **`tied/docs/agent-req-implementation-checklist.yaml`**). For the visual flow from session start through TDD and closing the loop, see **[tied/docs/methodology-diagrams.md](tied/docs/methodology-diagrams.md)** (especially Diagram 3 — PROC-TIED_DEV_CYCLE). For AI agent configuration and checklists, see **AGENTS.md** (copied into projects via `copy_files.sh`). Commit messages: [CONTRIBUTING.md](CONTRIBUTING.md) (TIED repo); projects get **tied/docs/commit-guidelines.md** as the quick reference (canonical in the TIED repo: **`tied/docs/commit-guidelines.md`**; installed via `copy_files.sh`).
...
- `scripts/yaml_tool.sh` — Canonicalize TIED YAML (`yq -i 'sort_keys(.. style="double")'`, one file per invocation; bool/int become string scalars), sort qualifying list groups (`--sort-lists`, Ruby; 2+ items), or optionally sort map keys (`--sort-keys` with `--sort-lists`); supports `-F/--find`, stdin, and NUL-separated paths (see **[PROC-YAML_EDIT_LOOP]** for lint).
- `scripts/lint_yaml.sh` — Backward-compatible lint-only entry point; delegates to `yaml_tool.sh`.
...
- `tools/agentstream/` — Go module **`stdd/agentstream`**: **`agentstream`** CLI plus packages for config, pipeline, executor, feature-spec batch, TDD loop YAML, checklist expansion, and **tied-yaml MCP preflight**. Build: `cd tools/agentstream && go build -o agentstream ./cmd/agentstream`. Optional **`--non-compact-html`** / **`--non-compact-html-indent`** reformat turn bodies to multi-line HTML; see [tools/agentstream/README.md](tools/agentstream/README.md) and TIED **REQ-GOAGENT-NON-COMPACT-HTML-FORMAT**.
...
- `tied/docs/pseudocode-writing-and-validation.md` - Unified IMPL pseudo-code guide (three-way alignment, phases A–I, LEAP, MCP/sidecar mechanics, `tied-cli`; see § Mechanics for sidecar editing)
...
- `tied/docs/requirement-list-state-guide-agent-workflow.md` - State guide workflow for MCP tool `requirement_list_state_guide`
- `tied/docs/ai-principles.md` - Agent operational mandates and checklists (canonical in the TIED repo; installed to client `tied/docs/` via `copy_files.sh` `DOCS_TO_COPY`)
...
- `tied/docs/processes.md` — TIED process reference copied to client `tied/docs/`; includes `[PROC-YAML_DB_OPERATIONS]`, `[PROC-YAML_EDIT_LOOP]`, `[PROC-TEST_STRATEGY]`, `[PROC-TIED_DEV_CYCLE]`, `[PROC-COMMIT_MESSAGES]`, `[PROC-RELEASE]`, `[PROC-CITDP]`, `[PROC-IMPL_CODE_TEST_SYNC]`, and `[PROC-SWIFT_BUILD]`
...
**For AI agents**: Use the TIED MCP server as the **primary** way to read and write TIED data. See [tied/docs/ai-agent-tied-mcp-usage.md](tied/docs/ai-agent-tied-mcp-usage.md) for the full directive (MCP-first; direct file access only when no tool supports the operation). For routing project TIED YAML writes through MCP (mandatory paths, failure playbook), see [tied/docs/yaml-update-mcp-runbook.md](tied/docs/yaml-update-mcp-runbook.md).
