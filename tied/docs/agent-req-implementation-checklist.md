...
2. Run `yq -i -P <file>` on each changed file (required for any **direct-edited** file; MCP-written files may still be normalized this way if your workflow runs it on all touched paths). This validates syntax and canonicalizes formatting in place.
