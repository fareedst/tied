...
4. When a bidirectional fidelity adapter is in scope, run the adapter check; mismatch is warn-only (same as unit-test-green); **CALL sub-adversarial-inquiry-pass** with `phase: verification` and `blocking: false`.
...
**Preconditions**: Caller supplies `scope`, `phase` (`pre_implementation|verification|close_out`), `proof_boundaries`, `blocking` (default `false`), and `profile_depth` (`minimal|integrated|strict_candidate`) selected before this call. Artifact root is `working/{REQ-TOKEN}/adversarial-inquiry`; generated evidence is not canonical TIED intent.
