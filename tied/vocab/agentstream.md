...
| **append-only bridge** | hook bridge, adherence hook bridge | Cursor: `mcp-server/dist/cli/adherence-append-action-attempted.js` via `.cursor/hooks/log.rb`. Claude: `mcp-server/dist/cli/claude-adherence-bridge.js` via bootstrap-merged `.claude/settings.json` `PostToolUse` → `.claude/hooks/tied-adherence-bridge.sh`. Both append `action_attempted` fail-silent when marker absent. [REQ-TIED_CLAUDE_ADHERENCE_HOOKS](../requirements/REQ-TIED_CLAUDE_ADHERENCE_HOOKS.yaml) |
