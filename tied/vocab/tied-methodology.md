...
| **recognized record list** | schema record array, mapping-record list | Mapping array under a **record-list registry** parent key where every element is a mapping with the configured stable sort field; sorted as whole blocks in TypeScript and Ruby |
| **record-list registry** | record list keys, schema-backed list registry | Normative table in `[ARCH-TIED_YAML_CANONICAL_PROFILE]`: `satisfaction_criteria` → `criterion`; `validation_criteria` → `method`; `alternatives_considered` → `name` |
