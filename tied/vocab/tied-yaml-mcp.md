...
| **Claude MCP safe merge** | repo mcp overwrite | Repo-root `.mcp.json` create-if-absent or add `mcpServers.tied-yaml` only when missing; Cursor create-only policy unchanged · [REQ-TIED_CLAUDE_HARNESS](../requirements/REQ-TIED_CLAUDE_HARNESS.yaml); Windows smoke asserts owned by [REQ-TIED_CLAUDE_BOOTSTRAP_OPS](../requirements/REQ-TIED_CLAUDE_BOOTSTRAP_OPS.yaml) |
