...
- **README** — [Repository Structure](README.md#repository-structure) § Scripts: documents root **`hello_world.sh`**, Go module **`stdd_hello_world_smoke`** / **`hello_world_script_entry_test.go`**, **`go test ./...`**, and TIED tokens **`REQ-GOAGENT-BASH-HELLO-SCRIPT`**, **`ARCH-GOAGENT-BASH-HELLO-SCRIPT`**, **`IMPL-GOAGENT-BASH-HELLO-SCRIPT`**.
...
- **Root Bash hello smoke + Go unit check** — Repository root **`hello_world.sh`** (strict Bash, single-line `Hello World!` output). **`go.mod`** (`module stdd_hello_world_smoke`) and **`hello_world_script_entry_test.go`** run `bash hello_world.sh` from the module root and assert stdout and exit 0. Project **`tied/`** documents **`REQ-GOAGENT-BASH-HELLO-SCRIPT`**, **`ARCH-GOAGENT-BASH-HELLO-SCRIPT`**, **`IMPL-GOAGENT-BASH-HELLO-SCRIPT`** (indexes, detail files, **`semantic-tokens.yaml`**) for agent checklist S13 sync and `tied_validate_consistency`.
