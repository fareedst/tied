...
- **`TIED.md`** and **`tied-language-spec.md`** — Removed from the repository root; they were never copied by **`copy_files.sh`**, so client projects never received them. **`AGENTS.md`**, **`ai-principles.md`**, **`processes.md`**, and **`docs/methodology-diagrams.md`** (and **`tied/`** mirrors where applicable) now point agents at methodology files that bootstrap does copy (e.g. **`tied/docs/LEAP.md`**, **`semantic-tokens.md`**). **README** no longer lists the removed files.
