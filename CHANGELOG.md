...
This release (when tagged) will summarize work that **(a)** makes **`tied/`** and **`tied/docs/`** the **only** in-tree locations for the same TIED guide and reference **`.md`**, **(b)** has **`copy_files.sh`** seed **`tied/processes.md` from `templates/processes.md`** and take other top-level guides from **`tied/`**, and **(c)** aligns **Go `agentstream`**, batch **shell** drivers, **AGENTS.md** / **preload** paths, and **IMPL-TIED** text with the **`tied/docs/agent-req-implementation-checklist.yaml`** and **`tied/docs/…`** entry points. Removed duplicate methodology files at the repo **root** and under **`docs/`** where they mirrored `tied/docs/`.

### Changed

- **Single canonical TIED methodology markdown** — Core guides and `tied/docs/*` in the TIED repository are **only** under **`tied/`** and **`tied/docs/`**; duplicate copies at the repo root and under **`docs/`** (for the same files) were removed. **`copy_files.sh`** sources guides from **`tied/*.md`** and reference docs from **`tied/docs/`** (not root or the old **`docs/`** mirror). **Go `agentstream`**, **`FindRepoRoot`**, and batch driver defaults use **`tied/docs/agent-req-implementation-checklist.yaml`**. **AGENTS.md**, **README**, **`tied/docs/ai-principles.md`** pathing, preload contracts, and IMPL-TIED copy descriptions updated.
- **`processes.md` template vs live** — **`templates/processes.md`** is the TIED-distributed bootstrap for new client projects; **`tied/processes.md`** in the TIED source repository is the **live** file (agents, **`processes_path`** in preload). **`copy_files.sh`** seeds **`tied/processes.md` from `templates/`** when missing on the client. The other four top-level guide **`.md`** files still copy from the TIED source tree’s **`tied/`**. The redundant **root** **`processes.md`** was removed. **IMPL-TIED_FILES** and **README** manual-mirror text updated.
...
- **`TIED.md`** and **`tied-language-spec.md`** — Removed from the repository root; they were never copied by **`copy_files.sh`**, so client projects never received them. **`AGENTS.md`** and methodology pointers now use **`tied/docs/ai-principles.md`**, **`tied/processes.md`**, **`tied/docs/methodology-diagrams.md`**, etc. **README** no longer lists the removed files.
...
- **processes.md** / **tied/processes.md** — Aligned process edits (see git diff).
- **detail-files-schema.md** / **tied/detail-files-schema.md** — Expanded **structural rules and common errors** for REQ/ARCH/IMPL detail files.
...
- **scripts/run-feature-batch.sh** — Default workspace is `.` (repo root); `agent-preload-contract.yaml` and `prompts/all.yaml` are optional when absent; `--prompt-file` is passed only when set; `set -x` / `set +x` around the Ruby runner; **`-f` / `--first-turn`** passthrough for mid-batch resume (requires `--session-id` when `N > 1`).
...
- **docs/agent-preload-contract-template.yaml** — `tied_paths`: `TIED_BASE_PATH`, `project_yaml_writable_globs`, `methodology_readonly_glob`, `mcp_tied_yaml_verified_this_session`.
...
- **docs/agent-req-implementation-checklist.md** — All reference and table paths updated to `tied/processes.md`, `tied/detail-files-schema.md`, `tied/docs/...` for sibling docs; session-bootstrap uses `tied/` for indexes and guides.
